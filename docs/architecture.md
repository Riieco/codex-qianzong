...
- `src-tauri/src/auth_vault.rs` stores the relay endpoint/API Key and the complete official `auth.json` snapshot in a versioned ChaCha20-Poly1305 vault. Windows stores the random master key in Credential Manager. macOS stores it beside the vault as `auth-vault.key` with `0600` permissions and never accesses Keychain; an existing keyring-encrypted vault is backed up before the local-key vault is created. The relay Key is bound to its endpoint; changing the endpoint requires a new Key.
